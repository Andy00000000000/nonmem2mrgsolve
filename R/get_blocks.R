...
get_block_pk <- function(ctl0 = NULL, mrg_code = NULL, cmts = NULL){
  ...
  input <- ctl0 %>%
    ...
    dplyr::mutate(V1 = gsub("\\s*\\.AND\\.\\s*","\\&\\&",V1))%>% #18mar2025 & to && to account for likely more common use case
    dplyr::mutate(V1 = gsub("\\s*\\.OR\\.\\s*","\\|\\|",V1))%>% #18mar2025 | to || to account for likely more common use case
